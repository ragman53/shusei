...
plan: 02
subsystem: pdf-ocr-pipeline
tags: [batch-processing, ocr, progress-tracking, parallel-execution]
dependency_graph:
  requires: ["03-01"]
  provides: ["batch-ocr-processing", "progress-tracking", "resume-support"]
  affects: ["src/core/db.rs", "src/core/pdf.rs", "src/core/ocr/engine.rs"]
tech_stack:
  added:
    - "futures 0.3"
    - "tokio macros feature"
  patterns:
    - "Semaphore-based concurrency control"
    - "Stream processing with buffer_unordered"
...
key_files:
  ...
  modified:
    ...
    - "Cargo.toml"
decisions:
  - "Used futures::stream::buffer_unordered for concurrency instead of tokio::spawn to avoid Send issues with rusqlite"
  - "Retry logic limited to 3 attempts per page before skipping"
  - "Batch size set to 10 pages for balance between memory and progress granularity"
  - "Progress tracking uses Unix epoch seconds for timestamps (consistent with books table)"
metrics:
  duration: ~45min
